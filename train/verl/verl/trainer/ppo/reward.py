...
def load_reward_manager(
    ...
    if reward_manager_cfg.source == "register" and reward_manager_cfg.name == "naive_multithreads_tool":
        try:
            from verl.trainer.constants import resolve_system_prompt

            model_system_prompt = resolve_system_prompt(config.data.get("system_prompt", None))
        except Exception:
            model_system_prompt = config.data.get("system_prompt", None)
        ...
        default_extra_info = {
            ...
            "max_total_response_length": rollout_cfg.get("max_total_response_length", 0),
            "overlong_buffer_len": config.reward_model.get("overlong_buffer_len", 0),
            "max_generation_round": rollout_cfg.get("max_generation_round", 5),
        }
        user_extra_info = dict(reward_kwargs.get("extra_info", {}))
        reward_kwargs["extra_info"] = {**default_extra_info, **user_extra_info}

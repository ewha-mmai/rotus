...
class RLHFDataset(Dataset):
    ...
    def __init__(
        ...
        self.system_prompt = config.get("system_prompt", None) # 0207 추가: 학습 데이터 json에 시스템 프롬프트 없을 경우, config에서 지정해서 추가하도록 함
        if self.system_prompt:
            try:
                from verl.trainer.constants import resolve_system_prompt

                self.system_prompt = resolve_system_prompt(self.system_prompt)
            except Exception:
                pass
    ...
    def maybe_filter_out_long_prompts(self, dataframe: datasets.Dataset = None):
        ...
        if self.filter_overlong_prompts:
            ...
            if processor is not None:
                ...
                def doc2len(doc) -> int:
                    try:
                        # 0207 추가: system prompt가 있을 경우 메시지에 추가해서 길이 계산
                        doc_for_len = doc.copy()
                        messages = doc[prompt_key]
                        if self.system_prompt:
                             # deep copy to avoid modifying original dataset during length check
                            messages = [{"role": "system", "content": self.system_prompt}] + copy.deepcopy(messages)

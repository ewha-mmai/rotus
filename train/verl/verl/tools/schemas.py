...
class ToolResponse(BaseModel):
    ...
    # Structured content for custom ordering of text/image/video.
    # When set, this is used directly as the message content instead of
    # auto-building from text/image/video fields.
    # Example: [{"type": "text", "text": "<tag>"}, {"type": "image"}, {"type": "text", "text": "</tag>"}]
    # 0207 작성: 기존 순서는 text-image-video 고정이었으나, Thyme의 경우 text-image-text 형태로도 content를 구성할 수 있어 이를 반영하기 위해 content 필드 추가
    ...
    def is_empty(self) -> bool:
        return not self.text and not self.image and not self.video and not self.content

    def is_text_only(self) -> bool:
        return self.text and not self.image and not self.video and not self.content

...
# 0207 추가: 기존 thyme 모델의 파이썬 샌드박스 코드 복사해옴
...
#TEMP_PROCESSED_IMAGES_DIR = "/mllm_hdd/yfzhang/data/temp_processed_images/" #"/data/ms-swift-latest/eval_o3_tool/tmp_image"
# For local testing, you might want to change this, e.g.:
TEMP_PROCESSED_IMAGES_DIR = "./temp_processed_images/"

class ReadOnlyPath:
    """
    一个上下文管理器，用于在代码块执行期间临时将文件设为只读。
    这可以防止沙盒代码修改原始输入文件。
    """
...
# However, in multi-threaded environments like Ray workers, signals are restricted.
# We set use_signals=False to use multiprocessing-based timeout.
@timeout_decorator.timeout(EXEC_TIME_LIMIT, use_signals=False)

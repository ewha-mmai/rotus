...
# 추가
@ToolParser.register("code_block")
class CodeBlockParser(ToolParser):
    """
    Parses code blocks (```python ... ``` or <code>...</code>) as calls to 'python_interpreter'.
    """

    def __init__(self, tokenizer) -> None:
        super().__init__(tokenizer)
        # Regex to capture code blocks.
        # Group 1: Markdown style ```python ... ```
        # Group 2: XML style <code> ... </code>
        self.code_block_regex = regex.compile(r"```python\s*(.*?)\s*```|<code>\s*(.*?)\s*</code>", regex.DOTALL | regex.IGNORECASE)
        ...
        for match in matches:
            ...
            if code_content and code_content.strip():
                ...
                if code_content.startswith("```python"):
                    ...
                    if code_content.endswith("```"):
                        code_content = code_content[:-3].strip()
                # Wrap as a tool call to 'python_interpreter'
                arguments = json.dumps({"code": code_content}, ensure_ascii=False)
                function_calls.append(FunctionCall(name="python_interpreter", arguments=arguments))

        content = self.code_block_regex.sub("", text)
